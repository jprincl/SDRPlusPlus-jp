...
short MY_B4_FFT::initial(int N) {
	...
	m_ifft_move_bit = m_M4<<1;  // ??????  /m_fwlen
...
short MY_B4_FFT::base4_fft(Complex_num *x, int sign) {
	...
	for (i = 1; i <= m_M4; i++) {
		...
		BlockLen = pow(4, i);		// ????????+????????????  interval_2
		BlockLen2 = BlockLen >> 2;	// ????????????  interval_1
		for (j = 0; j < BlockNum; j++) {	  // ??????????????4????????????
			for (k = 0; k < BlockLen2; k++) { // ??????????k??????
				F0 = k + j * BlockLen;		  // ????????????????????
				F1 = F0 + BlockLen2;
				F2 = F1 + BlockLen2;
				F3 = F2 + BlockLen2;
...
void MY_B4_FFT::fftfix(Complex_num *x, int sign) {
	...
	int i, j, k, u = 0, l = 0, wi = 0, n1, tr, ti, N = m_fwlen; //j????????????????????????????????????????
									 //k????????????????fft????????????log2??N??N????????????
									 //u ????????x[upper],l ????????x[lower]??wi????????????wn[wi]
	int SubBlockNum, SubBlockStep = 1;
	//SubBlockNum????k????????????SubBlockStep????k??????????????????????????????
	...
	for (k = N; k > 1; k = (k >> 1)) {				//????????????????log2(k)????????

		SubBlockNum = k >> 1;				//????????????????????????
		SubBlockStep = SubBlockStep << 1;	//??????????????????????????2??????????
		wi = 0;							//??????????????
		for (j = 0; j < SubBlockStep >> 1; j++) {	//????????????????j????j????????????????j????????
			//??????????????????????????????????wn??????????????????????wn
			for (u = j; u < N; u += SubBlockStep) {	//????????????????????????????????j??????????????????????
				//????????u??????(u>N)		
				l = u + (SubBlockStep >> 1);//????l????

				//??????????????????FFT  cos-isin
				tempr = (((__int64)x[l].real*m_Buffer_cos[wi]) >> 30) - (((__int64)x[l].imag*m_Buffer_sin[wi]) >> 30);
				//????x[u]=x[u]+x[l]*Wn,x[l]=x[u]-x[l]*Wn??????????
