...
// eibispace.de's sked-*.csv is NOT UTF-8 — verified directly against the
// A26 file, where station names with diacritics (e.g. "Rádio", "Córdoba",
// "Valparaíso") are single Windows-1252/Latin-1 bytes. Fed straight to
// ImGui (which requires UTF-8), those bytes are invalid sequences and
// render as the substitution glyph ('?' or tofu) — this converts each
// line to proper UTF-8 before anything else touches it.
//
...
std::string windows1252ToUtf8(const std::string& raw) {
...
std::vector<ListenInfoEntry> parseEibiCsv(const std::string& path) {
    ...
    while (std::getline(file, line)) {
        ...
        std::vector<std::string> f = splitSemicolons(windows1252ToUtf8(line));
